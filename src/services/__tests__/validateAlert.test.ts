jest.mock('../../helper', () => ({
	getStrategiesDB: jest.fn(() => {
		...
		// Simulate JsonDB push: updates the in-memory data structure
		const mockDb = {
...
jest.mock('@dydxprotocol/v3-client', () => ({
	Market: {
		'BTC-USD': 'BTC-USD',
		'ETH-USD': 'ETH-USD'
	}
		...
		build: jest.fn(() => ({
			client: {
				public: {
					getMarkets: jest.fn(() => ({
						markets: {
							'BTC-USD': { minOrderSize: '0.001' },
							'ETH-USD': { minOrderSize: '0.01' }
						}
					}))
				}
			}
	...
	describe('field validation', () => {
		...
		it('accepts valid positions: long, short, flat', async () => {
			for (const pos of ['long', 'short', 'flat']) {
				...
				// flat will be rejected for first order, but long/short should pass
				if (pos !== 'flat') {
					expect(result).toBe(true);
				}
			}
